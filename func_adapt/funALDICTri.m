...
function [U0,USubpb2,FSubpb2,udual,vdual,ConvItPerEle,ALSolveStep,coordinatesFEM,elementsFEM,dirichlet,neumann,mu,beta,...
    ALSub1Time,ALSub2Time] = funALDICTri(ImgNormalized,file_name,ImgSeqNum,gridxyROIRange,...
    ...
    if ImgSeqNum == 2 || StillFFTSearch == 1
        % ====== Integer Search ======
        [SizeOfFFTSearchRegion,x0,y0,x0Cen,y0Cen,u,v,uCen,vCen,cc,ccCen] = ...
            IntegerSearchTri(gridxROIRange,gridyROIRange,fNormalized,gNormalized,winsize,winstepsize,file_name);
        
        % ====== FEM mesh set up ======
        [coordinatesFEM,elementsFEM,coordinates,elements,dirichlet,neumann,x,y,xCen,yCen,M,N] = MeshSetUpTri2(x0,y0,x0Cen,y0Cen,winstepsize);
        % ====== Remove outliers ======
        ...
        % ====== Initial Value ======
        U0 = InitTri2(u,v,cc.max,uCen,vCen,ccCen.max,M,N); PlotuvInitTri; 
    else
...
%     disp('--- Remove bad points done ---')
    ...
    Plotdisp_showTri(USubpb1World,coordinatesFEMWorld,elementsFEM);
    Plotstrain_showTri(FSubpb1World,coordinatesFEMWorld,elementsFEM);
    ...
    Subpb2FDOrFEM = 2; % For tri-elements, only using FEM method for Subproblem 2;
    try
    ...
    %FSubpb1 = funSmoothStrain_adapt(FSubpb1,coordinatesFEM,elementsFEM,winstepsize/2,LevelNo,StrainFilterSize,StrainFilterStd);
    ...
    %%JY!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        for tempk = 1:length(betaList)
            beta = betaList(tempk);
            GaussPtOrder = 1; alpha = 0; [USubpb2] = Subpb2Tri(coordinatesFEM,elementsFEM,[],neumann,beta,mu,USubpb1,FSubpb1,udual,vdual,alpha,clusterNo);
            [FSubpb2,~,~] = funGlobal_CSTStrain(coordinatesFEM,elementsFEM,USubpb2);
            Err1(tempk) = norm(USubpb1-USubpb2,2);
            Err2(tempk) = norm(FSubpb1-FSubpb2,2);
        end
        ...
        % Using optimal beta to solve again
        [USubpb2] = Subpb2Tri(coordinatesFEM,elementsFEM,[],neumann,beta,mu,USubpb1,FSubpb1,udual,vdual,alpha,clusterNo);
    end
    ...
    Plotdisp_showTri(USubpb2World,coordinatesFEMWorld,elementsFEM);
    Plotstrain_showTri(FSubpb2World,coordinatesFEMWorld,elementsFEM);
    ...
    while (ALSolveStep < 3)
        ...
        %USubpb1(1:2:end) = reshape(u,size(elements,1),1); USubpb1(2:2:end) = reshape(v,size(elements,1),1);
        close all; %Plotuv(USubpb1,x0,y0); 
        %Plotdisp_showTri(USubpb1,coordinatesFEM,elementsFEM);
        save(['Subpb1_step',num2str(ALSolveStep)],'USubpb1','FSubpb1');
        ...
        if Subpb2FDOrFEM == 1 %FD
			...
			tic; [USubpb2] = Subpb2Tri(coordinatesFEM,elementsFEM,[],neumann,beta,mu,USubpb1,FSubpb1,udual,vdual,alpha,clusterNo);
        end
		ALSub2Time(ALSolveStep) = toc; toc
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if Subpb2FDOrFEM == 1 %FD
			...
			GaussPtOrder = 2; [FSubpb2] = funGlobal_CSTStrain(coordinatesFEM,elementsFEM,USubpb2);
		end
