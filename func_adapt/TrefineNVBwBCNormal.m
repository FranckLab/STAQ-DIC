function [coordinates,newElements,varargout] ...
    = TrefineNVBwBCNormal(coordinates,elements,varargin)

%TrefineNVB: local refinement of triangular mesh by newest vertex bisection, 
...
%    = TrefineNVB(coordinates,elements3,dirichlet,neumann,marked)
% or
%
% [coordinates,elements3] ...
%    = TrefineNVB(coordinates,elements3,marked)
%
%Comments:
%
%    TrefineNVB expects as input a mesh described by the 
...
%    Further elements will be refined by subsequently bisecting the newest
%    vertex to obtain a regular triangulation. 
% 
...
%*** Generate new nodes
edge2newNode(edge2newNode~=0)...
    = size(coordinates,1) + (1:nnz(edge2newNode));
...
for j = 1:nargout-2
    ...
    if ~isempty(boundary)
        ...
        if ~isempty(markedEdges)
            if j==1 % For dirichlet bc
                boundary = [boundary(~newNodes,:); ...
                boundary(markedEdges,1),newNodes(markedEdges); ...
                newNodes(markedEdges),boundary(markedEdges,2)];
            elseif j==2 % For neumann bc
                boundary = [boundary(~newNodes,:); ...
                boundary(markedEdges,1),newNodes(markedEdges),boundary(markedEdges,3:4); ...
                newNodes(markedEdges),boundary(markedEdges,2),boundary(markedEdges,3:4)];
            else
            end
        end
    end
...
newElements([idx(red),1+idx(red),2+idx(red),...
    3+idx(red)],:) = [tmp(red,[4,3,6]);tmp(red,...
    [1,4,6]);tmp(red,[4,2,5]); tmp(red,[3,4,5])];
