...
function [Uhat,EnrHAndTipEleIndex,EnrTipEleIndex] = Subpb2Tri_para(coordinatesFEM, elementsFEM, dirichlet, neumann, beta, mu, ...
...
while ConvergeOrNot < 0.5 && IterStep < 1
    ...
    parfor j = 1:size(elementsFEM,1)
        ...
        % ------ Construct A matrix ------
        tempA = TriArea*( (beta+alpha)*(DN')*(DN)+ mu*(N')*N ) ;
        %    if IterStep == 1
        %       A(tempIndexU,tempIndexU) = A(tempIndexU,tempIndexU) + TriArea*( (beta+alpha)*(DN')*(DN)+ mu*(N')*N ) ;
        %    end
        [IndexAXX,IndexAYY] = ndgrid(tempIndexU,tempIndexU);
        INDEXAIpar{j}=IndexAXX(:); INDEXAJpar{j}=IndexAYY(:); INDEXAVALpar{j}=tempA(:);
        
        % ------ Construct b vector ------
        % b(tempIndexU) = b(tempIndexU) + TriArea*( -beta*diag((DN')*(FMinusW(tempIndexF)')) + mu*NDiag*(UMinusv(tempIndexU)) +(alpha)*(DN')*DN*U(tempIndexU) );
        tempb = TriArea*( -beta*diag((DN')*(FMinusW(tempIndexF)')) + mu*NDiag*(UMinusv(tempIndexU)) +(alpha)*(DN')*DN*U(tempIndexU) );
