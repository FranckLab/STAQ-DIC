function [U,F] = funRemoveOutliersTri_adapt(coordinatesFEM,elementsFEM,U,F,Thr0,varargin)
% =========================================================================
...
Ux = U(1:2:end);
Trix = zeros(3,size(elementsFEM,1)); Triy = zeros(3,size(elementsFEM,1)); Tric = zeros(3,size(elementsFEM,1));
for j = 1:size(elementsFEM,1)
    Trix(1:3,j) = coordinatesFEM(elementsFEM(j,1:3),1);
    Triy(1:3,j) = coordinatesFEM(elementsFEM(j,1:3),2);
    Tric(1:3,j) = Ux(elementsFEM(j,1:3));
end
...
%     title('Click bad dispx-u points and press -Enter- after done','fontweight','normal')
    ...
    for tempk = 1:length(row) % iterate for each clicking points
        ...
        for templ = 1:size(elementsFEM,1)
            ismember1(templ) = ismember(DistCoordAllSortedInd(1),elementsFEM(templ,1:3));
            ismember2(templ) = ismember(DistCoordAllSortedInd(2),elementsFEM(templ,1:3));
            %ismember3(templ) = ismember(DistCoordAllSortedInd(3),elementsFEM(templ,1:3));
        end
        [~,coll] = find(ismember1+ismember2 == 2);
...
for j = 1:size(elementsFEM,1)
    Trix(1:3,j) = coordinatesFEM(elementsFEM(j,1:3),1);
    Triy(1:3,j) = coordinatesFEM(elementsFEM(j,1:3),2);
    Tric(1:3,j) = Uy(elementsFEM(j,1:3));
end
if size(elementsFEM,1)>2e4
    patch(Trix,Triy,Tric,'facecolor','interp','edgecolor','none'); 
else
...
%     Trix(1:3,j) = coordinatesFEM(elementsFEM(j,1:3),1);
%     Triy(1:3,j) = coordinatesFEM(elementsFEM(j,1:3),2);
%     Tric(1:3,j) = Ftemp(elementsFEM(j,1:3));
% end
% if size(elementsFEM,1)>2e4
%     patch(Trix,Triy,Tric,'facecolor','interp','edgecolor','none'); 
% else
