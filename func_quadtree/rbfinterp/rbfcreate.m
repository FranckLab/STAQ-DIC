...
switch lower(options.('RBFFunction'))
      case 'linear'          
        ...
        options.('rbfDphi')   = @rbfDphi_linear;
      case 'cubic'
        options.('rbfphi')   = @rbfphi_cubic;
        options.('rbfDphi')   = @rbfDphi_cubic;
      case 'multiquadric'
        options.('rbfphi')   = @rbfphi_multiquadrics;
        options.('rbfDphi')   = @rbfDphi_multiquadrics;
      case 'thinplate'
        options.('rbfphi')   = @rbfphi_thinplate;
        options.('rbfDphi')   = @rbfDphi_thinplate;
      case 'gaussian'
        options.('rbfphi')   = @rbfphi_gaussian;
        options.('rbfDphi')   = @rbfDphi_gaussian;
    otherwise
        options.('rbfphi')   = @rbfphi_linear;
        options.('rbfDphi')   = @rbfDphi_linear;
end
...
function du=rbfDphi_linear(r, const)
du=1;

function du=rbfDphi_cubic(r, const)
du=3*r.*r;

function du=rbfDphi_gaussian(r, const)
du= -1/(const*const) * r.*  exp(-0.5*r.*r/(const*const));

function du=rbfDphi_multiquadrics(r, const)
du= 1/(const*const) *r./sqrt(1+r.*r/(const*const));

function du=rbfDphi_thinplate(r, const)
du=2*r.*log(r+1) + r.^2./(r+1);
