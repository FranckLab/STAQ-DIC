...
[markInside4,markOutside4] = funMarkInside(coordinatesFEMQuadtree,elementsFEMQuadtree,DICpara.ImgRefMask);
...
U0NotNanInd = find(~isnan(U0(1:2:end)) & abs(U0(1:2:end))>1e-13);
U0Quadtree = 0*coordinatesFEMQuadtree(:);

dilatedI = ( imgaussfilt(double(Dg.ImgRefMask),1) );
dilatedI = logical( dilatedI > 0.01);
cc = bwconncomp(dilatedI,8);
indPxAll = sub2ind( Dg.imgSize, round(coordinatesFEMQuadtree(:,1)), round(coordinatesFEMQuadtree(:,2)) );
indPxNotNanAll = sub2ind( Dg.imgSize, round(DICmesh.coordinatesFEM(U0NotNanInd,1)),round( DICmesh.coordinatesFEM(U0NotNanInd,2)) );
...
for tempi = 1:length(stats)
    ...
    try
        ...
        % ------ Disp u ------
        op1 = rbfcreate( [DICmesh.coordinatesFEM(U0NotNanInd(LibList),1:2)]',[U0(2*U0NotNanInd(LibList)-1)]','RBFFunction', 'thinplate','RBFSmooth',1); %rbfcheck(op1);
        fi1 = rbfinterp( [coordinatesFEMQuadtree(LiaList,1:2)]', op1);
        U0Quadtree(2*LiaList-1) = fi1(:);

        % ------ Disp v ------
        op1 = rbfcreate( [DICmesh.coordinatesFEM(U0NotNanInd(LibList),1:2)]',[U0(2*U0NotNanInd(LibList))]','RBFFunction', 'thinplate','RBFSmooth',1); % rbfcheck(op1);
        fi1 = rbfinterp( [coordinatesFEMQuadtree(LiaList,1:2)]', op1);
