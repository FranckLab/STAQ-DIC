function PlotvelQuadtreePolarMasks(U,V,coordinatesFEM,elementsFEM,CurrentImg,CurrentImgMask,...
    DICpara,ImgSeqNum,Rnew,CircleFitPar)
%PLOTVELQUADTREE: to plot DIC solved displacement components  
%   PlotvelQuadtree(U,V,coordinatesFEMWorld,elementsFEM,CurrentImg,DICpara)
% ----------------------------------------------
%
...
vel_r = cos(theta).*vel_x + sin(theta).*vel_y;
vel_t = - sin(theta).*vel_x + cos(theta).*vel_y;
...
hold on; ax2=axes; h2=show([],elementsFEM(:,1:4),coordinatesFEMWorldDef/um2px,vel_r,'NoEdgeColor');
...
axis equal; axis tight; box on; set(gca,'fontSize',18); view(2);  set(gca,'ydir','normal');
hold on; ax2=axes; h2=show([],elementsFEM(:,1:4),coordinatesFEMWorldDef/um2px,vel_t,'NoEdgeColor');
set(gca,'fontSize',18); view(2); box on; axis equal;  axis tight;   
alpha(h2,OrigDICImgTransparency); colormap(cMap); caxis auto;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%% TODO: manually modify colormap and caxis %%%%%%
