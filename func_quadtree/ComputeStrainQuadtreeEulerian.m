...
switch DICpara.MethodToComputeStrain
    ...
    case 2 % Plane fitting
        ...
        try 
            for tempk=0:1, ULocal_Eulerian(2*markCoordHoleStrain-tempk) = nan; end
        catch
            for tempk=0:1, ULocal_Eulerian(2*markCoordHoleEdge-tempk) = nan; end
        end
...
%         pause;
        ...
        for tempi = 1:length(stats)
           ...
           if stats(tempi).Area > 20
                ...
                % figure, plot3(coordinatesFEM(LiaList,1),coordinatesFEM(LiaList,2),ULocal_Eulerian(2*LiaList),'.');
                ...
                [~,UPlaneFittemp,FPlaneFittemp] = funCompDefGrad2(uv, coordinatesFEM_Eulerian(row1234(LiaList),1:2), Rad, 1e6, dilatedI );
                
                % figure, plot3( coordinatesFEM(row1234(LiaList),1), coordinatesFEM(row1234(LiaList),2), ULocal_Eulerian(2*row1234(LiaList) ), '.' );
...
%         try
%             for tempk=0:3, FSubpb2(4*markCoordHoleStrain-tempk) = nan; end
%         catch
%             for tempk=0:3, FSubpb2(4*markCoordHoleEdge-tempk) = nan; end
%         end
        
        %%%%% Remove outliers %%%%%
        [~,F11RemoveOutlier] = rmoutliers(FSubpb2(1:4:end), 'movmedian', round(DICpara.winstepsize));
        [~,F21RemoveOutlier] = rmoutliers(FSubpb2(2:4:end), 'movmedian', round(DICpara.winstepsize));
        [~,F12RemoveOutlier] = rmoutliers(FSubpb2(3:4:end), 'movmedian', round(DICpara.winstepsize));
        [~,F22RemoveOutlier] = rmoutliers(FSubpb2(4:4:end), 'movmedian', round(DICpara.winstepsize));
        ...
        for tempj=1:4
            FSubpb2(4*F11RemoveOutlierInd-4+tempj) = nan;
            FSubpb2(4*F21RemoveOutlierInd-4+tempj) = nan;
            FSubpb2(4*F12RemoveOutlierInd-4+tempj) = nan;
            FSubpb2(4*F22RemoveOutlierInd-4+tempj) = nan;
        end
        ...
        if ~isempty(nanindex) || ~isempty(nanindexF)
            ...
            Ftemp = scatteredInterpolant(coordinatesFEM(notnanindex,1),coordinatesFEM(notnanindex,2),USubpb2(2*notnanindex-1),'natural','nearest');
            U1 = Ftemp(coordinatesFEM(row1234,1),coordinatesFEM(row1234,2));
            Ftemp = scatteredInterpolant(coordinatesFEM(notnanindex,1),coordinatesFEM(notnanindex,2),USubpb2(2*notnanindex),'natural','nearest');
            U2 = Ftemp(coordinatesFEM(row1234,1),coordinatesFEM(row1234,2));
            Ftemp = scatteredInterpolant(coordinatesFEM(notnanindexF,1),coordinatesFEM(notnanindexF,2),FSubpb2(4*notnanindexF-3),'natural','nearest');
            F11 = Ftemp(coordinatesFEM(row1234,1),coordinatesFEM(row1234,2));
            Ftemp = scatteredInterpolant(coordinatesFEM(notnanindexF,1),coordinatesFEM(notnanindexF,2),FSubpb2(4*notnanindexF-2),'natural','nearest');
            F21 = Ftemp(coordinatesFEM(row1234,1),coordinatesFEM(row1234,2));
            Ftemp = scatteredInterpolant(coordinatesFEM(notnanindexF,1),coordinatesFEM(notnanindexF,2),FSubpb2(4*notnanindexF-1),'natural','nearest');
            F12 = Ftemp(coordinatesFEM(row1234,1),coordinatesFEM(row1234,2));
            Ftemp = scatteredInterpolant(coordinatesFEM(notnanindexF,1),coordinatesFEM(notnanindexF,2),FSubpb2(4*notnanindexF-0),'natural','nearest');
...
%             end
        ...
        %%%%% Find elements near hole elements within distance Rad %%%%%
